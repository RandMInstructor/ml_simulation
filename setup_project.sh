...
install_command() {
    if [[ -n "${INSTALL_COMMANDS[$1]}" ]]; then
        if [[ "${INSTALL_COMMANDS[$1]}" == *".sh" ]]; then
            ...
            sudo apt install "${INSTALL_COMMANDS[$1]}" -y
        fi
    else
...
PREREQUISITES=("python3" "python3 -m venv -h")
...
# List of prerequisite commands
POSTREQUISITES=("python3")
# Check for all postrequisites
for cmd in "${POSTREQUISITES[@]}"; do
    check_command "$cmd"
done
